...
export class CharactersContainer {
    ...
    async resolveCharaDef (src: CharacterDef | number | undefined) {
      if (src === undefined) {
        ...
        if (ret === undefined) { throw new Error(`CharacterDefが見つからない: ${src}`) }
        return ret
      } else {
        return src
      }
    ...
    getUnusedID (name: string): string {
      ...
      while (true) {
        ...
        loops += 1
        if (loops > 100000) { throw new Error('much loops') }
        if (this.characters.has(id)) { continue }
        return id
        // break
      }
    }
...
export class CharacterReference {
    ...
    関係取得 (c: CharacterReference, createIfNothing:boolean = false) : UnidirectionalRelationship | undefined {
      if (!c || !c.ID) { throw new Error(`invalid character: ${(c || {}).ID}`) }
