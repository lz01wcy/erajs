...
class IO {
    ...
    async ensureNewline () {
      if (!this.currentLine) throw new Error()
      if (this.currentLine.childElementCount !== 0) { await this.newline() }
    }
    ...
    async newlineRaw (cls: string) {
      ...
      while (this.texts.childElementCount > 1000) {
        ...
        if (fc) { this.texts.removeChild(fc) }
    ...
    async printl (s?: string): Promise<void> {
      if (s !== undefined) { await this.prints(s) }
      await this.newline()
    }
    ...
    async printError (s?: string): Promise<void> {
      if (s !== undefined) { await this.printNormalRaw(s, 'pre', ['error']) }
      await this.newline()
    }
    ...
    private inputStringInternal (display: string): Promise<string> {
      return new Promise((resolve) => {
        for (const f of this.newerButtonStarters) { f() }
        ...
        const disp = this.inputStringEvent.addListener((str) => {
          ...
          for (const f of this.olderButtonDisposers) { f() }
    ...
    wait (): Promise<void> {
      return new Promise((resolve) => {
        ...
        const ok = () => {
          ...
          if (this.lastElement) { this.lastElement.textContent = '' }
          resolve()
        }
