    ...
    for (let i = 0; i < words.length; i++) {
      ...
      switch (word) {
        ...
        case 'Ａ':
          ...
          // 実行系
        case '愛撫':
          if (focused === '') { randomFocus() }
          if (Math.random() > 0.5) { T('撫でまわし') } else if (Math.random() > 0.5) { T('愛撫し') }
          if (isLast) { T('た。') } else { T('ながら') }
          if (Math.random() > 0.5) { focused = '' }
          break

        case 'キス':
          if (focused === '') { focused = 'Ｍ' }
          if (Math.random() > 0.5) { T('キスし') } else if (Math.random() > 0.5) { T('口付け') }
          if (isLast) { T('た。') } else { T('ながら') }
          if (Math.random() > 0.5) { focused = '' }
          break
      }
