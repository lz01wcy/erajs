...
export class TrainScene implements GameScene {
    ...
    constructor (public game: Game, data: any) {
      ...
      if (!this.playerIDs || this.playerIDs.length === 0) { throw new Error('playerIDsが指定されていない') }
      if (!this.targetIDs || this.targetIDs.length === 0) { throw new Error('targetIDsが指定されていない') }
    }
    ...
    async beginScene () {
      ...
      const player = this.game.characters.tryGet(this.playerIDs[0])
      const target = this.game.characters.tryGet(this.targetIDs[0])
      if (!player || !target) {
        let s = ''
