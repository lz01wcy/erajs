...
export const 同性判定 = (c1: CharacterReference, c2: CharacterReference) => {
  const c1v = c1.特性.Ｖ有り
  const c2v = c2.特性.Ｖ有り
  if (c1v && c2v) { return '女' } else if (!c1v && !c2v) { return '男' } else { return '異性' }
}
...
export const gaugeToJuel = (gaugeValue: number): number => {
  for (let i = 0; i < gaugeToJuelLevels.length; i++) {
    if (gaugeValue <= gaugeToJuelLevels[i]) { return gaugeToJuelAmounts[i] }
  }
...
const getOpponent = <T>(train: TrainStatus, c: CharacterReference, defaultValue: T) => {
  if (train.player === c) { return train.target } else if (train.target === c) { return train.player }
  return defaultValue
}
...
export class Train {
    ...
    async doTrain () {
      ...
      while (await this.stepTrain()) { break }
    ...
    playersName () {
      if (this.players.length === 0) { return '<誰もいない>' } else if (this.players.length > 1) { return `${this.players[0].呼び名}達` } else { return `${this.players[0].呼び名}` }
    }

    targetsName () {
      if (this.targets.length === 0) { return '<誰もいない>' } else if (this.targets.length > 1) { return `${this.targets[0].呼び名}達` } else { return `${this.targets[0].呼び名}` }
    }
}
/*
